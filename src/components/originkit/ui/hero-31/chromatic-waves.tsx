...
interface DottedBg2Props {
  /**
   * Whether the field animates.
   *
   * The delivered component hardcoded this to `true` while still shipping the
   * whole pause mechanism below — the ref, the rAF guard, and the zeroed speed
   * uniform were all unreachable. The parent computes this from reduced-motion,
   * intersection and tab visibility, so ignoring it meant a WebGL shader ran
   * forever behind a hidden tab and animated for readers who asked it not to.
   */
  play?: boolean;
  frequency?: number;
  speed?: number;
...
export default function DottedBg2({
  ...
  useEffect(() => {
     ...
     *
     * `new Renderer` throws "unable to create webgl context" when hardware
     * acceleration is off, when a driver is blocklisted, or simply when the tab
     * already holds as many contexts as the browser allows. This field is
     * decorative and aria-hidden, so the only correct response is to draw
     * nothing — the delivered component let the throw escape the effect, which
     * put Next's error overlay over the entire page and made a background
     * texture capable of taking the site down.
     */
    ...
    try {
      ...
      console.warn("Wave field disabled: WebGL is unavailable.", error);
      return;
    }
